        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {features.map((feature) => (
            <Link
              to={feature.link}
              key={feature.name}
              className="bg-white dark:bg-gray-900 rounded-lg border-2 border-gray-200 dark:border-gray-800 p-8 flex flex-col items-start text-left h-full transition-colors duration-200 group hover:border-gray-300 dark:hover:border-gray-700 active:bg-gray-50 dark:active:bg-gray-800"
            >
              <div className="mb-4">{feature.icon}</div>
              <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2 group-hover:text-[#00C6A7]">
                {feature.name}
              </h3>
              <p className="text-gray-600 dark:text-gray-400 text-sm min-h-[56px]">
                {' '}
                {feature.description}
              </p>
            </Link>
