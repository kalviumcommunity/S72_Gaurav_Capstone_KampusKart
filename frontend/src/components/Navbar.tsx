...
const Navbar: React.FC = () => {
  ...
  const toggleFeaturesDropdown = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
      ...
      {user && (
        <>
          ...
          >
            ...
            <div 
              id="features-menu"
              className={`${isFeaturesDropdownOpen ? 'block' : 'hidden'} md:absolute md:top-full md:left-0 bg-white shadow-lg rounded-md w-full md:w-48 z-50 mt-2 md:mt-0 overflow-hidden transition-all duration-200 ease-in-out`}
            >
