    ...
    <div className="space-y-8 pb-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {news.images && news.images.length > 0 ? (
          ...
          <div className="md:col-span-2 aspect-video bg-gray-50 dark:bg-gray-800 rounded-xl flex flex-col items-center justify-center border-2 border-dashed border-gray-200 dark:border-gray-700">
            <FiFileText className="w-12 h-12 text-gray-200 dark:text-gray-600 mb-2" />
      ...
      <div className="space-y-6">
        <div>
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <span className="px-4 py-1.5 rounded-full bg-teal-50 dark:bg-teal-900/30 text-[#00C6A7] dark:text-teal-300 text-xs font-bold uppercase tracking-wider border border-teal-100 dark:border-teal-700 flex items-center gap-2">
              <FiTag /> {news.category}
            </span>
            <span className="px-4 py-1.5 rounded-full bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-300 text-xs font-bold uppercase tracking-wider border border-blue-100 dark:border-blue-700 flex items-center gap-2">
              <FiCalendar />{' '}
      ...
      {isAdmin && (
        <div className="flex flex-wrap gap-4 pt-6 border-t-2 border-gray-100 dark:border-gray-700">
          <button
            onClick={() => onEdit(news)}
            className="flex-1 flex items-center justify-center gap-2 px-6 py-4 bg-[#181818] dark:bg-gray-700 text-white rounded-xl font-bold hover:bg-[#00C6A7] dark:hover:bg-teal-600 transition-all shadow-lg shadow-gray-100 dark:shadow-black/30"
          >
            <FiEdit2 /> Edit News
          </button>
          <button
            onClick={() => onDelete(news._id)}
            className="flex-none px-6 py-4 bg-white dark:bg-gray-900 text-red-600 dark:text-red-400 border-2 border-red-200 dark:border-red-700 rounded-xl font-bold hover:bg-red-50 dark:hover:bg-red-900/20 transition-all"
          >
